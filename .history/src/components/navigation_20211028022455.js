    ...
    return(
        ...
        <nav onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove} className={`Blog-Navigation`}>
            <span ref={menuIconRef} onClick={() => setMenuActive(!menuActive)} className={`menu-icon ${!menuActive && 'active'}`}>
                <IoIosMenu color={`${darkMode ? '#e8eaed' : "#000000"}`} style={{display: !menuActive ? "flex" : "none", color: menuActive ? "#383A23" : "null"}}/>
                <IoIosClose color={`${darkMode ? '#e8eaed' : "#000000"}`} style={{display: menuActive ? "flex" : "none", color: menuActive ? "#EC6103" : "null"}}/>
            </span>
