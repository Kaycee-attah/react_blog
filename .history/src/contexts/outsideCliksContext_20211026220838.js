...
export function ClickedOutsideProvider({ children }) {
    ...
    const handleClickOutsideInputField = useCallback(
        (e) => {
            if((inputRef.current === undefined) || (reactQuillRef.current === undefined)) {
                return;
            }  else if((inputRef.current.contains(e.target)) || (reactQuillRef.current.contains(e.target))) {
                return;
            } else {
    ...
    }, [handleClickOutsideInputField, handleClickOutsideTextEditorInputField])
