...
export function ClickedOutsideProvider({ children }) {
    ...
    const { setMoreIconSettingsMenu } = useContext(settingsContext);
    ...
    const handleClickOutsideMoreIconDropDown = useCallback(
        (e) => {
            if(dropDownMenuRef.current.contains(e.target)) {
                ...
                setMoreIconSettingsMenu(false);
            }
        },
        [setMoreIconSettingsMenu],
    );

    const handleClickOutsideSettingsDropDown = useCallback(
        (e) => {
            if(settingsDropdownRef.current.contains(e.target)) {
                return;
            } else if(settingsIconRef.current.contains(e.target)) {
                return;
            } else {
    ...
    useEffect(() => {
        ...
        document.addEventListener("mousedown", handleClickOutsideMoreIconDropDown);
        document.addEventListener("mousedown", handleClickOutsideCategoriesDropDown);
        document.addEventListener("mousedown", handleClickOutsideSettingsDropDown);
        ...
        return () => {
          ...
          document.removeEventListener("mousedown", handleClickOutsideMoreIconDropDown);
          document.removeEventListener("mousedown", handleClickOutsideCategoriesDropDown);
          document.removeEventListener("mousedown", handleClickOutsideSettingsDropDown);
        //   document.removeEventListener("mousedown", handleClickOutsideTextEditorInputField);
        }
    }, [handleClickOutsideInputField, handleClickOutsideSideBar, handleClickOutsideMoreIconDropDown, handleClickOutsideCategoriesDropDown, handleClickOutsideSettingsDropDown])
