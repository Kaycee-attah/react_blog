...
export function ClickedOutsideProvider({ children }) {
    ...
    const handleClickOutsideInputField = useCallback(
        (e) => {
            if((inputRef.current === undefined) || (textEditorInputRef === undefined)) {
                return;
            }  else if((inputRef.current.contains(e.target)) || (textEditorInputRef.current.contains(e.target))) {
                return;
            } else {
    ...
    //     if(textEditorInputRef === undefined) {
    //         return;
    //     } else if(textEditorInputRef.current.contains(e.target)) {
    //         return;
    //     } else if{
    ...
    }, [handleClickOutsideInputField, inputActive])
